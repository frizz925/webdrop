...
export const encodeBuffer = (buf: ArrayBuffer | BufferSource | Uint8Array) =>
...
export const encodeCipherParams = (params: AesGcmParams) =>
	Object.assign({}, params, { iv: encodeBuffer(params.iv) }) as EncodedCipherParams;
export const decodeCipherParams = (params: EncodedCipherParams) =>
	Object.assign({}, params, { iv: decodeBuffer(params.iv) }) as AesGcmParams;
...
export const encodeKDFParams = (params: Pbkdf2Params) =>
	Object.assign({}, params, { salt: encodeBuffer(params.salt) }) as EncodedKDFParams;
export const decodeKDFParams = (params: EncodedKDFParams) =>
	Object.assign({}, params, { salt: decodeBuffer(params.salt) }) as Pbkdf2Params;
...
export const importMasterKey = (raw: ArrayBuffer) =>
	crypto.subtle.importKey('raw', raw, 'AES-KW', false, MASTER_KEY_USAGES);
...
export const maybeDecryptObject = async <C extends Content>(
	obj: FileObject<EncryptedContent | C>
) => {
	const config = getCryptoConfig();
	...
	const { masterKey } = config;
	const subKey = await subtle.unwrapKey(
		'raw',
		decodeBuffer(wrappedKey),
		masterKey,
		'AES-KW',
		SUB_KEY_PARAMS,
		false,
	...
	const payload = await subtle.decrypt(
		decodeCipherParams(cipher),
		subKey,
	...
	const result = Object.assign({}, obj, decoded) as FileObject<C>;
	return result;
};
